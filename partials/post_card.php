...
<div class="tweet-card mb-4 p-3 rounded" id="post-<?= $post['id'] ?>" data-post-id="<?= $post['id'] ?>" data-username="@<?= $post['username'] ?>">
  ...
  <div class="mb-3 pb-3 border-bottom border-secondary">
    ...
    <?php if (!empty($post["video_path"])): ?>
      <div class="tweet-media-wrapper">
        <video controls class="tweet-media rounded-4 shadow-sm" style="max-width: 100%;">
          <source src="<?= BASE_URL ?>/assets/posts/<?= htmlspecialchars($post["video_path"]) ?>?t=<?= time() ?>" type="video/mp4">
  ...
  <div class="d-flex justify-content-start align-items-center gap-3 mb-3">
    <form action="<?= BASE_URL ?>/controllers/like_post.php" method="POST" class="me-2 d-inline">
      <input type="hidden" name="post_id" value="<?= $post["id"] ?>">
      <button type="submit" class="btn btn-sm <?= !empty($post["liked_by_me"]) ? 'btn-light text-dark' : 'btn-primary' ?>">
        <i class="bi bi-hand-thumbs-up me-1"></i>
        <?= !empty($post["liked_by_me"]) ? 'Gefällt' : 'Gefällt mir' ?>
        <?php if (!empty($post["like_count"])): ?>
          <span class="ms-2 text-dark"><?= $post["like_count"] ?></span>
        <?php endif; ?>
      </button>
    </form>

    <button type="button" class="btn btn-outline-light btn-sm toggle-comment-form" data-post-id="<?= $post['id'] ?>">
  ...
  <div class="comment-form d-none mt-3 px-2" id="comment-form-<?= $post['id'] ?>">
    <form method="POST" class="comment-form-inner d-flex align-items-start gap-2 mb-3" data-post-id="<?= $post["id"] ?>">
      <img class="rounded-circle mt-1" src="<?= BASE_URL ?>/assets/uploads/<?= $_SESSION["profile_img"] ?>" alt="Profilbild" style="width: 32px; height: 32px;">
